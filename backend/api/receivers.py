...
def game_or_sandbox_save(sender, instance: Game | Sandbox, created, **kwargs):
    ...
    if created:
        ...
        """
        if isGame and instance.started == True:
            _first_adjudication(instance)
        ...
        # Check for if it was just started
        if isGame:
            if instance.started and instance.next_adjudication is None: # was just started
                _first_adjudication(instance)

post_save.connect(game_or_sandbox_save, sender=Game)
post_save.connect(game_or_sandbox_save, sender=Sandbox)